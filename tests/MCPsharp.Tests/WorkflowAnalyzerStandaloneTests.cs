...
");
    ...
    public async Task FindWorkflowsAsync_FindsYmlFiles()
    {
        ...
        Assert.That(w => w.EndsWith("build.yml", Does.Contain(workflows)));
        Assert.That(w => w.EndsWith("deploy.yml", Does.Contain(workflows)));
    }
    ...
    public async Task GetAllWorkflowsAsync_ReturnsAllWorkflows()
    {
        ...
        Assert.That(workflows.Count, Is.EqualTo(2));
        Assert.That(w => w.Name == "Build", Does.Contain(workflows));
