...
{{
    ...
    public async Task ParseMigrationFile_ShouldReturnValidMigrationInfo_ForValidMigrationFile()
    {
        ...
        Assert.That(result.Operations, Does.Contain(o => o.Type.Equals("CreateTable", StringComparison.OrdinalIgnoreCase)));
    ...
    public async Task AnalyzeMigrations_ShouldDetectMigrations_WhenMigrationsExist()
    {
        ...
        Assert.That(result.Migrations, Does.Contain(m => m.Name == "CreateTestTable"));
        Assert.That(result.Migrations, Does.Contain(m => m.Name == "AddColumnToTestTable"));
