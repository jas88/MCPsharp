...
public class DerivedService : IService
{
    ...
    public async Task FindCallers_ShouldFindDirectCallers()
    {
        ...
        Assert.That(c => c.CallerType.Contains("Consumer", Does.Contain(result.Callers)));
    }
    ...
    public async Task FindIndirectCallers_ShouldIncludeInterfaceCalls()
    {
        ...
        Assert.That(result, Is.Not.Null);
        Assert.That(result.TotalCallers >= 1, Is.True);
        Assert.That(c => c.CallType == Assert.That(CallType.Indirect, Does.Contain(result.Callers));
    }
