...
public class McpToolRegistryTests : IDisposable
{
    ...
    public void GetTools_EachToolHasValidNameDescriptionAndSchema()
    {
        ...
        foreach (var tool in tools)
        {
            ...
            Assert.That(schemaRoot.TryGetProperty("type", out var Assert.That(typeProperty, Is.True));
    ...
    public async Task ExecuteTool_ProjectOpen_WithInvalidPath_Fails()
    {
        ...
        Assert.That(result.Error, Assert.That(StringComparison.OrdinalIgnoreCase, Does.Contain("not exist"));
    }
    ...
    public async Task ExecuteTool_ProjectInfo_WithoutOpenProject_Fails()
    {
        ...
        Assert.That(result.Success, Is.False);
        Assert.That(result.Error, Is.Not.Null);
        Assert.That(result.Error, Assert.That(StringComparison.OrdinalIgnoreCase, Does.Contain("No project"));
    }
