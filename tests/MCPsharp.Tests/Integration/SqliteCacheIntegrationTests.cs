...
public class SqliteCacheIntegrationTests
{
    ...
    public async Task FullWorkflow_ProjectOpenToResourceQuery()
    {
        ...
        result.Contents[0].Text, Does.Contain("Project Overview"));
        result.Contents[0].MimeType, Is.EqualTo("text/markdown"));
        ...
        result.Contents[0].Text, Does.Contain("project"));
    }
    ...
    public async Task Database_PersistsSymbolsAcrossOperations()
    {
        ...
        foundSymbols.Select(s => s.Name), Does.Contain("TestClass"));
        foundSymbols.Select(s => s.Name), Does.Contain("TestMethod"));
    ...
    public async Task Database_CallChainAnalysis()
    {
        ...
        callChain.Select(s => s.Name), Does.Contain("MethodB"));
        callChain.Select(s => s.Name), Does.Contain("MethodC"));
        ...
        backwardChain.Select(s => s.Name), Does.Contain("MethodA"));
        backwardChain.Select(s => s.Name), Does.Contain("MethodB"));
    }
    ...
    public void Prompts_ProvidePracticalGuidance()
    {
        ...
        analyzePrompt.Messages[0].Content.Text, Does.Contain("find_symbol"));
        analyzePrompt.Messages[0].Content.Text, Does.Contain("semantic"));

        implementPrompt.Messages[0].Content.Text, Does.Contain("user authentication"));
        implementPrompt.Messages[0].Content.Text, Does.Contain("find_symbol"));

        fixPrompt.Messages[0].Content.Text, Does.Contain("null reference exception"));
        fixPrompt.Messages[0].Content.Text, Does.Contain("find_call_chains"));

        // Verify prompt structure
        Assert.That(analyzePrompt.Messages, Has.Count.EqualTo(1));
        analyzePrompt.Messages[0].Role, Is.EqualTo("user"));
        analyzePrompt.Messages[0].Content.Type, Is.EqualTo("text"));
    }
    ...
    public void Resources_ListAllAvailable()
    {
        ...
        result.Resources.Select(r => r.Uri), Does.Contain("project://overview"));
        result.Resources.Select(r => r.Uri), Does.Contain("project://structure"));
        result.Resources.Select(r => r.Uri), Does.Contain("project://guidance"));
    ...
    public async Task Database_StaleFileDetection()
    {
        ...
        // Assert
        staleFiles.Select(f => f.RelativePath), Does.Contain("file2.cs"));
        staleFiles.Select(f => f.RelativePath).Should().NotContain("file1.cs");

        deletedFiles.Select(f => f.RelativePath), Does.Contain("file3.cs"));
    ...
    public async Task Database_ProjectStructureKeyValue()
    {
        ...
        Assert.That(keys, Does.Contain("solution_info"));
    ...
    public async Task Integration_FullProjectAnalysisWorkflow()
    {
        ...
        symbolSearch.Any(s => s.Name == "UserService"), Is.True);
        symbolSearch.Any(s => s.Name == "GetUser"), Is.True);

        // Note: Field name mismatch means project name may not be shown
        resourceRead.Contents[0].Text, Does.Contain("Project Overview"));

        Assert.That(projectInfo, Is.Not.Null);
        projectInfo!["rootPath"], Is.EqualTo(_testProjectPath));
        projectInfo["fileCount"], Is.TypeOf<int>());
    }
    ...
    public async Task Database_FindCallersAndCallees()
    {
        ...
        callers[0].Caller.Name, Is.EqualTo("Caller"));
        callers[0].Reference.ReferenceKind, Is.EqualTo("Call"));

        Assert.That(callees, Has.Count.EqualTo(1));
        callees[0].Callee.Name, Is.EqualTo("Callee"));
        callees[0].Reference.ReferenceKind, Is.EqualTo("Call"));
    }
    ...
    public void PromptRegistry_ListsAllBuiltInPrompts()
    {
        ...
        result.Prompts.Any(p => p.Name == "analyze-codebase"), Is.True);
        result.Prompts.Any(p => p.Name == "implement-feature"), Is.True);
        result.Prompts.Any(p => p.Name == "fix-bug"), Is.True);
    ...
    public async Task ProjectContextManager_TracksMultipleFiles()
    {
        ...
        context.KnownFiles.Any(f => f.Contains("Test.cs")), Is.True);
        context.KnownFiles.Any(f => f.Contains("Program.cs")), Is.True);
    }
