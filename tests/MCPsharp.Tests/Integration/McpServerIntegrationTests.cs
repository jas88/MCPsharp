...
public class McpServerIntegrationTests : IDisposable
{
    ...
    public void Test01_BasicProjectWorkflow_OpenAndListFiles()
    {
        ...
        Assert.That(listResult.Files, Does.Contain(f => f.RelativePath.Contains("Program.cs")));
        Assert.That(listResult.Files, Does.Contain(f => f.RelativePath.Contains("Helper.cs")));
        Assert.That(listResult.Files, Does.Contain(f => f.RelativePath.Contains("README.md")));
    }
    ...
    public async Task Test15_DirectoryCreation_NestedDirectories()
    {
        ...
        File.Exists(Path.Combine(_testProjectRoot, fileName)), Is.True);
    ...
    public async Task Test19_ConcurrentOperations_MultipleReads()
    {
        ...
        results.Should().AllSatisfy(r => Assert.That(r.Success, Is.True));
